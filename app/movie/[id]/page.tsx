...
import { getMovieById, getMovieVideos, pickTrailer } from "../../lib/movies";
    ...
    <main className="min-h-screen bg-[#040816] text-white">
      <section className="relative overflow-hidden">
        <div className="absolute inset-0">
          <Image
            src={`${process.env.NEXT_PUBLIC_TMDB_IMAGE_URL}${movie.backdrop_path}`}
          ...
          <div className="flex flex-col gap-8 lg:flex-row lg:items-end">
            <div className="mx-auto w-full max-w-[260px] lg:mx-0 lg:max-w-[320px]">
              <div className="overflow-hidden rounded-[2rem] border border-white/10 bg-white/10 p-2 shadow-[0_30px_80px_rgba(15,23,42,0.55)] backdrop-blur-xl">
                <Image src={`${process.env.NEXT_PUBLIC_TMDB_IMAGE_URL}${movie.poster_path}`}
            ...
            <div className="flex-1 rounded-[2rem] border border-white/10 bg-slate-950/45 p-6 shadow-[0_20px_80px_rgba(2,6,23,0.45)] backdrop-blur-xl sm:p-8 lg:p-10">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                {trailer ? (
                  ...
                  <button
                    disabled
                    className="cursor-not-allowed rounded-full bg-gradient-to-r from-violet-600/50 to-cyan-500/50 px-5 py-3 text-sm font-semibold text-white/70 shadow-lg shadow-violet-900/20"
                  >
                    ▶ No Trailer Available
                  </button>
                )}
                <button className="rounded-full border border-violet-400/30 bg-violet-500/10 px-5 py-3 text-sm font-semibold text-violet-100 transition hover:border-cyan-400/40 hover:bg-cyan-400/10">
                  ❤ Add to Favorites
                </button>
        ...
        <div className="grid gap-6 lg:grid-cols-[1.7fr_1fr]">
          <div className="rounded-[1.75rem] border border-white/10 bg-slate-950/55 p-6 shadow-[0_20px_60px_rgba(0,0,0,0.25)] backdrop-blur-xl sm:p-8">
            ...
            <p className="mt-4 text-base leading-8 text-slate-300">
              The film layers urban mythology, emotional tension, and sleek visual storytelling into a modern thriller that feels intimate, immersive, and impossible to look away from.
            </p>
          </div>
          ...
          <div className="rounded-[1.75rem] border border-white/10 bg-gradient-to-br from-violet-950/80 to-slate-950/70 p-6 shadow-[0_20px_60px_rgba(0,0,0,0.25)] backdrop-blur-xl sm:p-8">
            ...
            <div className="mt-6 grid gap-4 text-sm sm:grid-cols-2">
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                ...
                <p className="mt-1 font-semibold text-white">{movie.budget}</p>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                <p className="text-slate-400">Revenue</p>
                <p className="mt-1 font-semibold text-white">{movie.revenue}</p>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                <p className="text-slate-400">Status</p>
                <p className="mt-1 font-semibold text-white">{movie.status}</p>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                <p className="text-slate-400">Original Language</p>
                <p className="mt-1 font-semibold text-white">{movie.original_language}</p>
              </div>
            </div>
          </div>
        </div>
